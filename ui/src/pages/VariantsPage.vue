...
// The model declares only "/" as a section link, so its inferred `Href`
// union carries just that literal — the cast is a widening to the route
// `app.ts` actually registers, not a claim about the model's own type.
function goToParents() {
  void app.navigateTo("/parents" as unknown as Parameters<typeof app.navigateTo>[0]);
}
</script>
...
<template>
  <PlBlockPage title="Antibody Variant Designer">
    <template #append>
      <PlBtnGhost @click.stop="goToParents">
        Parents
        <template #append>
          <PlMaskIcon24 name="arrow-right" />
        </template>
      </PlBtnGhost>
