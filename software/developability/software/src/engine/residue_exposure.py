...
def _load_max_sasa_per_residue(path: Path) -> dict[str, float]:
    ...
    with path.open() as fh:
        for line in fh:
            ...
            if len(parts) < 2:
                continue
            max_sasa[parts[0]] = float(parts[1])
    return max_sasa


_AXA_REFS: dict[str, float] = _load_max_sasa_per_residue(_REFS_PATH)
...
def compute_rsasa(pdb_path: str) -> dict[tuple[str, str], float | None]:
    ...
    matching `residue_index.Residue.imgt`.
...
def annotate(
    residues: Iterable[residue_index.Residue], pdb_path: str
