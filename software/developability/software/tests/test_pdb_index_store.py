...
class TestPdbIndexRoundTrips:
    def test_write_then_read_returns_equal_entries(self, tmp_path):
        entries = [
            pdb_index_store.Entry(clonotype_key="clone-1", filename="clone-1.pdb"),
            pdb_index_store.Entry(clonotype_key="clone-2", filename="clone-2.pdb"),
        ]
        path = tmp_path / "pdb_index.tsv"

        pdb_index_store.write_index(str(path), entries)

        assert pdb_index_store.read_index(str(path)) == entries
    ...
    def test_file_order_is_preserved_not_re_sorted(self, tmp_path):
        ...
        entries = [
            pdb_index_store.Entry(clonotype_key="b", filename="b.pdb"),
            pdb_index_store.Entry(clonotype_key="a", filename="a.pdb"),
        ]
        path = tmp_path / "pdb_index.tsv"

        pdb_index_store.write_index(str(path), entries)

        assert [e.clonotype_key for e in pdb_index_store.read_index(str(path))] == ["b", "a"]

    def test_empty_index_round_trips_to_empty(self, tmp_path):
        path = tmp_path / "pdb_index.tsv"

        pdb_index_store.write_index(str(path), [])

        assert pdb_index_store.read_index(str(path)) == []


class TestStemIsTheOnlyFilenameSource:
    def test_stem_drops_the_extension(self):
        entry = pdb_index_store.Entry(clonotype_key="clone-1", filename="clone-1.pdb")
    ...
    def test_a_clonotype_key_holding_json_punctuation_still_resolves_to_a_safe_stem(
        ...
        entry = pdb_index_store.Entry(clonotype_key='["odd/key"]', filename="antibody-07.pdb")
        path = tmp_path / "pdb_index.tsv"

        pdb_index_store.write_index(str(path), [entry])
        [rehydrated] = pdb_index_store.read_index(str(path))
