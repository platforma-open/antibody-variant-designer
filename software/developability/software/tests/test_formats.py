...
the rejection reason, and which residues came out researchable. A format that is
not in `CLAUDE.md` must reach a named rejection here, never a partial index.
"""
...
class TestCase1Nanobody:
    ...
    def test_whole_chain_is_researched(self, batch):
        rejection, residues = run_structure(
            batch, remarks("H", "H") + "\n" + make_pdb(v_domain("H"))
        )
...
class TestCase3ScFv:
    """VH + linker + VL on one chain — a named rejection, not a partial index."""

    def test_two_v_domains_on_one_chain_rejection(self, batch):
        rejection, residues = run_structure(
            batch,
            remarks("H", "A") + "\n" + remarks("L", "A") + "\n"
        ...
        assert rejection == "structure-multi-domain-chain"
        assert residues == []

    def test_two_roles_naming_one_chain_rejection(self, batch):
        # The role collision alone is enough, even when the numbering
        # happens not to repeat.
        rejection, _ = run_structure(
            batch,
            remarks("H", "A") + "\n" + remarks("L", "A") + "\n"
            + make_pdb(v_domain("A")),
        )
...
class TestCase6HalfMab:
    ...
    def test_a_file_with_no_variable_domain_skips(self, batch):
        rejection, residues = run_structure(batch, make_pdb(c_domain("H", 1)))

        assert rejection == "no-researchable-residue"
        assert residues == []
